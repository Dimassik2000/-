...
with open('top_results.txt', 'w', encoding='utf-8') as file:
    for item in result_top:
        file.write(f"{item['name']}, {item['city']}, {item['system']}, {item['time_on_game']} \n")

result_stat = get_stat_by_time_on_game(db)
with open('stat_results.txt', 'w', encoding='utf-8') as file:
    file.write(f"Sum: {result_stat['sum']} \n")
    file.write(f"Avg: {result_stat['avg']} \n")
    file.write(f"Min: {result_stat['min']} \n")
    file.write(f"Max: {result_stat['max']} \n")

result_freq = get_freq_by_tours_count(db)
with open('freq_results.txt', 'w', encoding='utf-8') as file:
    for item in result_freq:
        file.write(f"Tours Count: {item['tours_count']} \n")

result_filter = filter_by_tours_count(db, 10)
with open('filter_results.txt', 'w', encoding='utf-8') as file:
