...
def write_field_statistics(db, field, file_name):
    cursor = db.cursor()
...
def write_categorical_field_frequency(db, field, file_name):
    cursor = db.cursor()
...
write_field_statistics(db, "duration_ms", "field_statistics.json")

# Задача 3: Вывод частоты встречаемости для категориального поля
write_categorical_field_frequency(db, "artist", "categorical_field_frequency.json")
